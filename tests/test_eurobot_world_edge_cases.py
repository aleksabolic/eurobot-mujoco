...
def test_action_arguments_are_clamped():
    ...
    world.step_blue((int(Verb.PICK), world.N + 5, 99, 1))
    _drain_events(world)
    assert int(world.blue.node) == last_node

    # move back to first pickup using negative index (should clamp to zero)
    world.step_blue((int(Verb.PICK), -10, -3, 1))
    _drain_events(world)
    assert int(world.blue.node) == 0

    world.step_blue((int(Verb.PICK), first_pickup, int(Col.BLUE), 1))
    _drain_events(world)
...
def test_steal_blocked_when_disallowed():
    ...
    reward += _drain_events(world)
    assert reward <= -world.rewards.invalid_action_penalty
...
def test_time_penalty_applies_per_elapsed_time():
    ...
    world = _make_world(rewards=cfg)
    start = world.t_left
    target = world.PICKUPS[-1]
    reward, _ = world.step_blue((int(Verb.PICK), target, int(Col.BLUE), 1))
    reward += _drain_events(world)
