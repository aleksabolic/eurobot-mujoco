...
def test_step_move_then_pick(env):
    ...
    # move
    obs, reward, done, _, _ = env.step(np.array([Verb.MOVE, pickup, Col.BLUE, 0]))
    _drain_env(env)
    obs = env._obs()
    assert not done
    assert env.world.blue.node == pickup
    # pick
    obs, reward, done, _, _ = env.step(np.array([Verb.PICK, pickup, Col.BLUE, 1]))
    _drain_env(env)
    obs = env._obs()
...
def test_done_when_time_runs_out(env):
    ...
    obs, reward, done, _, _ = env.step(np.array([Verb.WAIT, env.world.blue.node, Col.BLUE, 0]))
    assert done
