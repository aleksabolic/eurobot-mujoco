...
class EurobotCV2Renderer:
    def __init__(self, world: EurobotWorld, size=(900, 600)):
        self.world = world
        self.W, self.H = size
        self.pad = 12
        self.scale = min((self.W-2*self.pad)/(TABLE_X_MAX-TABLE_X_MIN),
                         (self.H-2*self.pad)/(TABLE_Y_MAX-TABLE_Y_MIN))
        self.ox = self.pad - TABLE_X_MIN*self.scale
        self.oy = self.pad - TABLE_Y_MIN*self.scale
    ...
    def draw_snapshot(self, idx=-1, show=False):
        ...
        for k, idx_node in enumerate(self.world.PANTRIES):
            ...
            cv2.rectangle(img, p0, p1, BLK, 1)
            cv2.putText(img, f"B{b}/Y{y}/N{n}", (p0[0]+4, p0[1]+self._l2p(0.12)),
        ...
        for k, idx_node in enumerate(self.world.PICKUPS):
            ...
            cv2.putText(img, f"B{b} Y{y}", (px - self._l2p(0.07), py + self._l2p(0.05)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, BLK, 1, cv2.LINE_AA)
        ...
        # header
        cv2.putText(img, f"t_left = {t_left:.1f}s", (10, 20),
